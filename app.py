...
def preprocess_excel_remove_uncertain_icds():
    """Preprocess the Excel file to remove ICD codes from A. Coded Diagnoses Table when they exist in comb table"""
    try:
        excel_file = os.path.join(DATA_DIR, 'parsed_results_new_v6.xlsx')
        if not os.path.exists(excel_file):
            logger.warning(f"Excel file {excel_file} not found - skipping preprocessing")
            return
        
        logger.info("Starting Excel preprocessing to remove uncertain ICD codes...")
        ...
        for index, row in df.iterrows():
            ...
            combination_code_review_str = row.get('comb', '')
            
            logger.info(f"Processing account {account_num} - Coded diagnoses: {bool(coded_diagnoses_str)}, Comb: {bool(combination_code_review_str)}")
            
            if not coded_diagnoses_str or not combination_code_review_str:
                logger.info(f"Skipping account {account_num} - missing data")
                continue
            
            # Parse the arrays
            coded_diagnoses = safe_parse_array(coded_diagnoses_str)
            combination_code_review = safe_parse_array(combination_code_review_str)
            
            logger.info(f"Account {account_num} - Parsed coded diagnoses: {len(coded_diagnoses) if coded_diagnoses else 0}, comb: {len(combination_code_review) if combination_code_review else 0}")
            
            if not coded_diagnoses or not combination_code_review:
                logger.info(f"Skipping account {account_num} - empty parsed data")
                continue
            
            # Get uncertain ICD codes from comb table
            uncertain_icd_codes = set()
            for uncertain_item in combination_code_review:
                uncertain_code = uncertain_item.get('icd-10-cm code', '') or uncertain_item.get('icd_code', '') or uncertain_item.get('code', '')
                if uncertain_code:
                    uncertain_icd_codes.add(str(uncertain_code))
            
            logger.info(f"Account {account_num} - Found uncertain ICD codes: {uncertain_icd_codes}")
            
            if not uncertain_icd_codes:
                continue
            
            # Filter out uncertain ICD codes from coded diagnoses
            original_count = len(coded_diagnoses)
            filtered_diagnoses = []
            removed_codes = []
            
            logger.info(f"Account {account_num} - Checking {original_count} coded diagnoses against uncertain codes")
            
            for diagnosis in coded_diagnoses:
                icd_code = diagnosis.get('ICD-10-CM Code', '')
                logger.info(f"Account {account_num} - Checking ICD code: {icd_code} against uncertain codes: {uncertain_icd_codes}")
                if icd_code not in uncertain_icd_codes:
                    filtered_diagnoses.append(diagnosis)
                else:
                    removed_codes.append(icd_code)
                    total_removed += 1
                    logger.info(f"Account {account_num} - REMOVING uncertain ICD code: {icd_code}")
            
            # Update the row if changes were made
            if removed_codes:
                accounts_processed += 1
                logger.info(f"Account {account_num}: Removed {len(removed_codes)} uncertain ICD codes: {removed_codes}")
                
                # Convert filtered diagnoses back to string format
                filtered_diagnoses_str = str(filtered_diagnoses)
                
                # Update the DataFrame
                df.at[index, 'A. Coded Diagnoses Table'] = filtered_diagnoses_str
        
        # Save the modified Excel file
        if total_removed > 0:
            df.to_excel(excel_file, index=False)
            logger.info(f"Excel preprocessing completed: {total_removed} ICD codes removed from {accounts_processed} accounts")
            logger.info(f"Modified Excel file saved: {excel_file}")
        else:
            logger.info("No uncertain ICD codes found - no changes made to Excel file")
            
    except Exception as e:
        logger.error(f"Error during Excel preprocessing: {e}")
...
def check_sdx_match_filter(icd_code, account_num, csv_data):
    ...
    try:
        ...
        # For SDX Mismatch, return unmapped codes (codes not matched by any LLM code)
        # We need to get all LLM SDX codes for this account to determine unmapped ones
        parsed_data = load_parsed_results_data()
        account_parsed_data = parsed_data.get(account_num, {})
        combination_code_review = account_parsed_data.get('combination_code_review', [])
        
        # Get all LLM SDX codes
        llm_sdx_codes = set()
        for item in combination_code_review:
            if item.get('type', '').upper() == 'SDX':
                llm_code = item.get('icd-10-cm code', '')
                if llm_code:
                    llm_sdx_codes.add(str(llm_code))
        
        # Find unmapped ground truth codes (not matched by any LLM code)
        unmapped_codes = []
        for gt_code in ground_truth_codes:
            is_mapped = False
            # Check direct match
            if gt_code in llm_sdx_codes:
                is_mapped = True
            else:
                # Check specificity matches
                for llm_code in llm_sdx_codes:
                    if len(gt_code) > 1 and len(llm_code) > 1:
                        if gt_code[:-1] == llm_code[:-1] or gt_code.startswith(llm_code) or llm_code.startswith(gt_code):
                            is_mapped = True
                            break
            
            if not is_mapped:
                unmapped_codes.append(gt_code)
        
        return {"status": "SDX Mismatch", "gt_codes": unmapped_codes}
    except Exception as e:
...
def load_icd_review_data_from_parsed():
    """Load ICD review data from parsed A. Coded Diagnoses Table, excluding uncertain ICDs"""
    try:
        if os.path.exists(os.path.join(DATA_DIR, 'parsed_results_new_v6.xlsx')):
            ...
            for _, row in df.iterrows():
                ...
                # Parse the arrays of dictionaries
                coded_diagnoses = safe_parse_array(coded_diagnoses_str)
                combination_code_review = safe_parse_array(combination_code_review_str)
                ...
                # Convert parsed diagnoses to review format (no filtering needed since Excel is preprocessed)
                processed_codes = []
                
                for diagnosis in coded_diagnoses:
                    icd_code = diagnosis.get('ICD-10-CM Code', '')
                    processed_codes.append(icd_code)
                    review_item = {
                        'icd_code': icd_code,
                ...
                logger.info(f"Account {account_num} - Processed codes: {processed_codes}")
            
            logger.info(f"Loaded ICD review data for {len(icd_data)} accounts from parsed diagnoses (Excel preprocessed)")
            return icd_data
        else:
...
def calculate_account_statistics(account_num, parsed_data, csv_data, comparison_data):
    ...
    try:
        ...
        # FIXED: matched_icds should be the same as matched_sdx for SDX accuracy
        matched_icds = matched_sdx
        
        # Calculate SDX accuracy based on matched SDX vs ground truth SDX
        accuracy = round((matched_sdx / total_sdx * 100) if total_sdx > 0 else 0, 1)
...
# Preprocess Excel file to remove uncertain ICD codes BEFORE loading any data
preprocess_excel_remove_uncertain_icds()

# Load completed accounts on startup BEFORE creating SAMPLE_ACCOUNTS
load_completed_accounts()

def get_uncertain_icd_codes(account_num, parsed_data):
    """Extract uncertain ICD codes from the combination_code_review data for a specific account"""
    uncertain_icd_codes = set()
    
    if account_num in parsed_data:
        account_data = parsed_data[account_num]
        combination_code_review_str = account_data.get('comb', '')
        
        if combination_code_review_str:
            combination_code_review = safe_parse_array(combination_code_review_str)
    ...
    return uncertain_icd_codes

def filter_out_uncertain_icds(review_items, uncertain_icd_codes):
    """Filter out review items that have ICD codes in the uncertain list"""
    if not uncertain_icd_codes:
        return review_items
    ...
    for item in review_items:
        ...
        if icd_code not in uncertain_icd_codes:
            ...
            logger.info(f"Filtering out uncertain ICD code {icd_code}")
    ...
    for account_num, parsed_data in PARSED_RESULTS_DATA.items():
        ...
        if documents and (total_sdx > 0 or pdx_coded_count > 0):
            SAMPLE_ACCOUNTS[account_num] = {
                ...
                "uncertain_icd_codes": get_uncertain_icd_codes(account_num, parsed_data),
...
# Global variables for time tracking and account locking
audit_sessions = {}  # Store session start times: {account_id: {user: start_time}}
account_locks = {}  # Store account locks: {account_id: {'user': user_id, 'timestamp': timestamp, 'session_id': session_id}}
...
def lock_account(account_id, user_id):
    """Lock an account for a specific user"""
    try:
        ...
        logger.info(f"Account {account_id} locked by user {user_id}")
        return True
    except Exception as e:
        logger.error(f"Error locking account {account_id}: {e}")
        return False

def unlock_account(account_id, user_id=None):
    """Unlock an account (only if locked by the same user or if user_id is None)"""
    try:
        if account_id in account_locks:
            if user_id is None or account_locks[account_id]['user'] == user_id:
                del account_locks[account_id]
                logger.info(f"Account {account_id} unlocked by user {user_id}")
                return True
            else:
                logger.warning(f"User {user_id} attempted to unlock account {account_id} locked by {account_locks[account_id]['user']}")
                return False
        return True
    except Exception as e:
...
def is_account_locked(account_id, user_id=None):
    """Check if an account is locked and optionally by whom"""
    if account_id not in account_locks:
        return False, None
    
    lock_info = account_locks[account_id]
    
    # Check if lock is expired (30 minutes)
    lock_age = (datetime.now() - lock_info['timestamp']).total_seconds() / 60
    if lock_age > 30:
        logger.info(f"Account {account_id} lock expired after {lock_age:.1f} minutes, auto-unlocking")
        unlock_account(account_id)
        return False, None
    
    # If user_id is provided, check if it's locked by this user
    if user_id and lock_info['user'] == user_id:
        return False, None  # Not locked for this user
    
    return True, lock_info['user']
...
def cleanup_expired_locks():
    ...
    try:
        ...
        for account_id, lock_info in account_locks.items():
            lock_age = (current_time - lock_info['timestamp']).total_seconds() / 60
            if lock_age > 30:
                expired_locks.append(account_id)
        
        for account_id in expired_locks:
            unlock_account(account_id)
...
def logout():
    ...
    if 'user_id' in session:
        ...
        # Unlock all accounts locked by this user
        accounts_to_unlock = []
        for account_id, lock_info in account_locks.items():
            if lock_info['user'] == username:
                accounts_to_unlock.append(account_id)
        
        for account_id in accounts_to_unlock:
            unlock_account(account_id, username)
        
        if accounts_to_unlock:
            logger.info(f"User {username} logged out - unlocked {len(accounts_to_unlock)} accounts")
...
def index():
    ...
    # Add lock status to accounts
    for account_id in pending_accounts_data:
        is_locked, locked_by = is_account_locked(account_id)
        pending_accounts_data[account_id]['is_locked'] = is_locked
        pending_accounts_data[account_id]['locked_by'] = locked_by if is_locked else None
...
def review(account_id):
    ...
    # Check if account is locked by another user
    user_id = session.get('user_id')
    is_locked, locked_by = is_account_locked(account_id, user_id)
    
    if is_locked:
        flash(f'This account is currently being reviewed by {locked_by}. Please try again later.', 'warning')
        return redirect(url_for('index'))
    
    # Lock the account for this user
    if not lock_account(account_id, user_id):
        flash('Failed to lock account. Please try again.', 'error')
        return redirect(url_for('index'))
    
    # Start audit time tracking
    if account_id not in audit_sessions:
        audit_sessions[account_id] = {}
    audit_sessions[account_id][user_id] = datetime.now()
    logger.info(f"User {user_id} started reviewing account {account_id}")
    if account:
        ...
        # Pass the account object to the template (no filtering needed since Excel is preprocessed)
...
def submit_feedback(account_id):
    ...
    try:
        ...
        # Unlock the account after successful submission
        user_id = session.get('user_id')
        unlock_account(account_id, user_id)
        logger.info(f"Account {account_id} unlocked after feedback submission by {user_id}")
...
@app.route('/api/unlock_account/<account_id>', methods=['POST'])
@login_required
def unlock_account_api(account_id):
    """API endpoint to manually unlock an account"""
    try:
        user_id = session.get('user_id')
        if unlock_account(account_id, user_id):
            logger.info(f"Account {account_id} manually unlocked by {user_id}")
            return jsonify({'status': 'success', 'message': f'Account {account_id} unlocked successfully'})
        else:
            return jsonify({'status': 'error', 'message': 'Failed to unlock account. You may not have permission to unlock this account.'}), 403
    except Exception as e:
        logger.error(f"Error unlocking account {account_id}: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/account_locks')
@login_required
def get_account_locks():
    ...
    try:
        ...
        locks_info = {}
        for account_id, lock_info in account_locks.items():
            locks_info[account_id] = {
                'locked_by': lock_info['user'],
                'locked_at': lock_info['timestamp'].isoformat(),
                'lock_age_minutes': (datetime.now() - lock_info['timestamp']).total_seconds() / 60
            }
        return jsonify({'locks': locks_info})
    except Exception as e:
        logger.error(f"Error getting account locks: {e}")
...
def get_random_unreviewed():
    ...
    # Get only pending (unreviewed) accounts
    available_accounts = list(pending_accounts)
    
    # Filter out accounts that are currently locked by other users
    unlocked_accounts = []
    for account_id in available_accounts:
        is_locked, locked_by = is_account_locked(account_id)
        if not is_locked:
            unlocked_accounts.append(account_id)
    
    if not unlocked_accounts:
        logger.warning("No unlocked pending accounts available for random selection")
        return jsonify({
            'error': 'No accounts available',
            'message': 'All pending accounts are currently being reviewed by other users. Please try again later.'
        }), 404
    
    random_account = random.choice(unlocked_accounts)
    
    logger.info(f"Random unreviewed account selected: {random_account} from {len(unlocked_accounts)} available accounts")
    return jsonify({
            ...
            if not df.empty:
                ...
                for _, row in df.sort_values('timestamp', ascending=False).iterrows():
                    try:
                        ...
                        # Handle CC/MCC value properly
                        cc_mcc_value = row.get('cc_mcc', '')
                        if pd.isna(cc_mcc_value) or cc_mcc_value == '' or str(cc_mcc_value).lower() == 'nan':
                            cc_mcc_value = ''
                        else:
                            cc_mcc_value = str(cc_mcc_value)
...
@app.route('/validation/download_excel')
@login_required
def download_validation_excel():
    """Download validation data as Excel file"""
    try:
        feedback_file = os.path.join(DATA_DIR, 'feedback_data.xlsx')
        ...
        filename = f'validation_data_{timestamp}.xlsx'
        
        # Save to a temporary file
        temp_file = os.path.join(DATA_DIR, filename)
        ...
        logger.error(f"Error downloading validation Excel: {e}")
        return jsonify({'error': str(e)}), 500
...
def lookup_account(account_id):
    ...
    # Allow lookup for any account that has feedback data (no filtering needed since Excel is preprocessed)
